...
export class StreamDock293sV3 extends StreamDock {
  KEY_COUNT = 18
  ...
  setDevice(): void {
    ...
    this.featureOption.deviceType = DeviceType.Dock293sV3
  }

  getImageKey(logicalKey: ButtonKey): number {
    const hw = IMAGE_KEY_MAP.get(logicalKey)
    if (hw === undefined) throw new Error(`StreamDock293sV3: unsupported key ${logicalKey}`)
    return hw
  }
