...
import { InputEvent, ButtonKey, EventType, DeviceInfo } from '../types/InputTypes'
...
export class StreamDockM18 extends StreamDock {
  KEY_COUNT = 15
  ...
  setDevice(): void {
    ...
    this.featureOption.ledCounts = 24
    this.featureOption.deviceType = DeviceType.DockM18
  }

  getImageKey(logicalKey: ButtonKey): number {
    const hw = IMAGE_KEY_MAP.get(logicalKey)
    if (hw === undefined) throw new Error(`StreamDockM18: unsupported key ${logicalKey}`)
    return hw
  }
  ...
  keyImageFormat(): ImageFormat { return { size: [64, 64], format: 'JPEG', rotation: 0, flip: [false, false] } }
  touchscreenImageFormat(): ImageFormat { return { size: [480, 272], format: 'JPEG', rotation: 0, flip: [false, false] } }
}
